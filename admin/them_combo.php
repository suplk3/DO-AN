...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ten === '') {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO combos (ten, mo_ta, gia, active) VALUES (?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "ssii", $ten, $mo_ta, $gia, $active);
...
<div class="form-container">
    ...
    <form method="POST">
        <div class="form-group">
            ...
            <input type="text" name="ten" required placeholder="Ví dụ: Combo 1 Bắp 1 Nước" value="<?= htmlspecialchars($_POST['ten'] ?? '') ?>">
        </div>
        <div class="form-group">
            <label>Mô tả chi tiết</label>
            <textarea name="mo_ta" rows="4" placeholder="Ví dụ: 1 bắp phô mai + 1 pepsi lớn"><?= htmlspecialchars($_POST['mo_ta'] ?? '') ?></textarea>
        </div>
        <div class="form-group">
            <label>Giá (VNĐ) *</label>
            <input type="number" name="gia" required placeholder="75000" min="0" value="<?= htmlspecialchars($_POST['gia'] ?? '') ?>">
        </div>
        <div class="form-group" style="display:flex; align-items:center; gap: 10px;">
            <input type="checkbox" name="active" id="active" checked style="width:18px; height:18px;">
